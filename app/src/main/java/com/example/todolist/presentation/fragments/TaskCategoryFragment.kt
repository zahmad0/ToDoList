...
class TaskCategoryFragment : ParentFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.apply {
            text.text = category
            statusChipGroup.setOnCheckedStateChangeListener { chipGroup, i ->
                val id = i[0]
                val chip = chipGroup.findViewById(id) as Chip
                when (chip.tag) {
                    "0" ->  viewModel.getUncompletedTaskOfCategory(category).observe(viewLifecycleOwner) {
                        loadData(it)
                    }
                    "1" -> viewModel.getCompletedTaskOfCategory(category).observe(viewLifecycleOwner) {
                        loadData(it)
                    }
                    else ->  viewModel.getAllTaskOfCategory(category).observe(viewLifecycleOwner) {
                        loadData(it)
                    }
                }
            }
        }
        viewModel.getAllTaskOfCategory(category).observe(viewLifecycleOwner) {
            loadData(it)
        }
    }
